...
{1. Curso de engenharia da computação - Iniciação à Pesquisa.
 ...
 % XXX estou realmente na dúvida:
 %III. Heurística Estática para Times Cooperativos de Robôs.
 III. Título.
...
\imprimirfolhadeaprovacao{17 de julho de 2014}
% ---
...
% DONE[jansegre] Dividir em 3 parágrafos:
% - Tema
% - Objetivo
% - Contrubuições
...
possível uso dos seguintes algoritmos: redes neurais, lógica nebulosa,
otimização da colônia de formigas, algoritmo genético e recozimento simulado, e
também, propõe um modelo para abstrair o comportamento de um time desconhecido.
% TODO[jansegre] inserir ponto sobre conclusão

A grande contribuição deste trabalho é concluir que a lógica nebulosa não é
adequada para abordar o problema devido à complexidade de incorporar o conceito
de temporalidade. Além disso, a otimização de colônia de formigas, o algoritmo
genético e o recozimento simulado são mais apropriados para apoio de um
algoritmo baseado em rede neural. É proposto que em um trabalho futuro seja
implementado um algoritmo com rede neural e esse usado num algoritmo minimax
para efetuar a movimentação do adversário.
...
%
\textbf{Palavras-chaves}: inteligência artificial, redes neurais, robótica.
\end{resumo}
...
The goal of this work is to study ways to, given the log of a match, foresee
...
The major contribution of this work is concluding that fuzzy logic is not
adequate to approach the problem given the complexity to incorporate time
awareness into it, furthermore the ant colony optimization, genetic algorithm
and simulated annealing only have use to support an algorithm, while the main
algorithm is best implemented with a neural network. It is suggested that a
future work implements a neural network algorithm to be used on a minimax to
make the moves of the rival team.
...
\input{partes/def_problema}
%\input{partes/rev_bibliografica}
